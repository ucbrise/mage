...
namespace mage::dsl {
    inline void range_from_addr(VirtualPageRange& result, VirtAddr addr, BitWidth width, std::uint8_t page_shift) {
        result.start = addr >> page_shift;
        result.end = (addr + width - 1) >> page_shift;
    }

    bool merge_range(VirtualPageRange& r1, const VirtualPageRange& r2) {
    ...
    void calculate_input_ranges(std::vector<VirtualPageRange>& ranges, const VirtualInstruction& v, std::uint8_t page_shift) {
    ...
    InstructionNumber annotate_input(VirtAddr input, InstructionNumber this_use, std::unordered_map<VirtualPageNumber, InstructionNumber>& next_access, std::uint8_t page_shift) {
        if (input == invalid_vaddr) {
            return invalid_instr;
        }
        VirtualPageNumber page = input >> page_shift;
    ...
    std::uint64_t reverse_annotate_program(std::string reverse_annotated_program, std::string original_program, std::uint8_t page_shift) {
        ...
        std::vector<VirtualPageRange> input_ranges;

        std::unordered_map<VirtualPageNumber, InstructionNumber> next_access;
        for (InstructionNumber i = original->num_instructions - 1; i != UINT64_MAX; i--) {
            const VirtualInstruction& v = original->instructions[i];

            VirtualPageRange output_range;
            ...
            for (int j = 0; j != input_ranges.size(); j++) {
                VirtualPageRange& r = input_ranges[j];
            ...
            for (int j = 0; j != input_ranges.size(); j++) {
                VirtualPageRange& r = input_ranges[j];
                for (VirtualPageNumber page = r.start; page != r.end + 1; page++) {
            ...
            for (VirtualPageNumber page = output_range.end; page != output_range.start - 1; page--) {
