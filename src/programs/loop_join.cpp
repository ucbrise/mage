...
namespace mage::programs::loop_join {
    ...
    void create_loop_join_circuit(const ProgramOptions& args) {
        ...
        ShardedArray<Record<key_width, record_width>> table1(args.problem_size, args.worker_index, args.num_workers, Layout::Blocked);
        table1.for_each([=](std::size_t i, auto& input) {
            input.data.mark_input(Party::Garbler);
        });

        ShardedArray<Record<key_width, record_width>> table2(args.problem_size, args.worker_index, args.num_workers, Layout::Blocked);
        table2.for_each([=](std::size_t i, auto& input) {
