/*
 * This file is based on util/aes_opt.h in EMP-toolkit, which appears to be
 * based on other code.
 */
...
#ifndef MAGE_CRYPTO_AES_CPU_HPP_
#define MAGE_CRYPTO_AES_CPU_HPP_
...
namespace mage::crypto {
     ...
     */
    static inline void AES_ks2_index(block random, std::uint64_t idx, ROUND_KEYS *KEYS) {
        block user_key[2];
        user_key[0] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[1] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);

        AES_ks2(user_key, KEYS);
    }

    static inline void AES_ks4_index(block random, std::uint64_t idx, ROUND_KEYS *KEYS) {
        block user_key[4];
        user_key[0] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[1] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);
        idx++;
        user_key[2] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[3] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);

        AES_ks4(user_key, KEYS);
    }

    static inline void AES_ks8_index(block random, std::uint64_t idx, ROUND_KEYS *KEYS) {
        block user_key[8];
        user_key[0] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[1] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);
        idx++;
        ...
        user_key[3] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);
        idx++;
        user_key[4] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[5] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);
        idx++;
        user_key[6] = xorBlocks(makeBlock(2 * idx, (std::uint64_t)0), random);
        user_key[7] = xorBlocks(makeBlock(2 * idx + 1, (std::uint64_t)0), random);
